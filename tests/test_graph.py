...
def _test_dijkstra(flag=gamera.graph.FREE):
    ...
    correct_paths = {
        gamera.graph.FREE: {
            8: {8: (0.0, [8]), 1: (7.0, [1, 8]), 2: (15.0, [2, 7, 1, 8]), 6: (6.0, [6, 8]), 7: (12.0, [7, 1, 8])},
            1: {8: (7.0, [8, 7, 1]), 1: (0.0, [1]), 2: (8.0, [2, 7, 1]), 6: (9.0, [6, 2, 7, 1]), 7: (5.0, [7, 1])},
            2: {8: (4.0, [8, 7, 2]), 1: (11.0, [1, 8, 7, 2]), 2: (0.0, [2]), 6: (1.0, [6, 2]), 7: (2.0, [7, 2])},
            6: {8: (4.0, [8, 6]), 1: (11.0, [1, 8, 6]), 2: (19.0, [2, 7, 1, 8, 6]), 6: (0.0, [6]),
                7: (16.0, [7, 1, 8, 6])},
            7: {8: (2.0, [8, 7]), 1: (9.0, [1, 8, 7]), 2: (3.0, [2, 7]), 6: (4.0, [6, 2, 7]), 7: (0.0, [7])}}
...
def _test_fully_connected(flag=gamera.graph.FREE):
    ...
    assert g.is_fully_connected() is False
    ...
    assert g.is_fully_connected() is True
...
def _test_add_nodes(flag=gamera.graph.FREE, count=2000):
    ...
    for i in range(1000, 1000 + count):
        assert g.has_node(i) is False

    for i in range(1000, 1000 + count):
        g.add_node(i)
        assert g.nnodes == i + 1 - 1000
        assert g.has_node(i) is True
        assert g.get_node(i)() == i

    assert g.has_node(int(1000 + count / 2)) is True
...
def _test_add_nodes_sequence(flag=gamera.graph.FREE, count=2000):
    ...
    assert g.has_node(500) is False
...
def _test_large_graph(flag=gamera.graph.FREE, count=20000):
    ...
    for i in range(0, count):
        assert g.has_node(i) is True
        assert g.has_edge(i, i + 1) is True
        assert g.get_node(i)() == i
...
def _test_memory(flag=gamera.graph.FREE, count=100):
    ...
    for i in range(0, count):
        ...
        assert g.has_node(200003 + i) is True
...
def test_textline_reading_order():
    ...
    correct_orders = {
        "data/reading_order_2.png": [(42, 40, 1462, 114), (42, 158, 683, 232), (42, 276, 683, 350), (42, 418, 683, 492),
                                     (42, 560, 683, 633), (822, 158, 1462, 633), (42, 701, 1462, 775),
                                     (42, 843, 494, 917), (562, 843, 1132, 917), (42, 985, 683, 1059),
                                     (822, 985, 1132, 1059), (1200, 843, 1462, 1059)],
        "data/reading_order.png": [(51, 56, 1471, 130), (51, 174, 691, 248), (51, 292, 691, 366), (51, 434, 691, 508),
                                   (51, 576, 691, 649), (830, 174, 1471, 508), (830, 576, 1471, 649),
                                   (51, 717, 1471, 791), (51, 859, 691, 933), (51, 1001, 691, 1075),
                                   (830, 859, 1471, 933), (830, 1001, 1471, 1075)]
    }
